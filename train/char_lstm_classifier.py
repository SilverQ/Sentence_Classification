...
class TextClassifier:
    def __init__(self, hparams, data_dir):
        ...
        with np.load(os.path.join(self.hparams.glove_dir, "glove.6B.300d.trimmed.npz")) as pretrained_data:
            ...
            print('Word Embedding shape: ', np.shape(self.word_embeddings))
    ...
    def _inference(self, inputs: tf.Tensor, lengths: tf.Tensor, char_inputs: tf.Tensor, char_lengths: tf.Tensor):
        ...
        print("output_dim : ", output_dim)      # output_dim :  9, label.vocab의 엔트리 갯수.

        char_vocab_size = len(self.id2char)
        print("char_vocab_size : ", char_vocab_size)    # char_vocab_size :  57
        ...
        print("char_embeddings : ", char_embeddings)
        # char_embeddings :  <tf.Variable 'inference/char_embeddings:0' shape=(57, 50) dtype=float32_ref>

        char_embedded = tf.nn.embedding_lookup(char_embeddings, char_inputs)
        print("char_embedded : ", char_embedded)
        # char_embedded :  Tensor("inference/embedding_lookup/Identity:0", shape=(?, ?, ?, 50), dtype=float32)
        ...
        print("char_embedded : ", char_embedded)
        # char_embedded :  Tensor("inference/Reshape:0", shape=(?, ?, 50), dtype=float32)

        char_lengths = tf.reshape(char_lengths, [-1])
        print("char_lengths : ", char_lengths)
        # char_lengths :  Tensor("inference/Reshape_1:0", shape=(?,), dtype=int32)
        ...
        with tf.variable_scope("char-bi-RNN"):
            ...
            print("char_hiddens : ", char_hiddens)
            # char_hiddens :  Tensor("inference/char-bi-RNN/concat:0", shape=(?, 100), dtype=float32)

            char_hiddens = tf.reshape(char_hiddens,
                                  [tf.shape(char_inputs)[0], tf.shape(char_inputs)[1], self.hparams.char_embedding_dim*2])
            print("char_hiddens : ", char_hiddens)
            # char_hiddens :  Tensor("inference/char-bi-RNN/Reshape:0", shape=(?, ?, 100), dtype=float32)
        ...
        # word_embedded :  Tensor("inference/embedding_lookup_1/Identity:0", shape=(?, ?, 300), dtype=float32)
        # default.json의 설정값 300

        char_word_inputs = tf.concat([word_embedded, char_hiddens], axis=-1)
        print("char_word_inputs : ", char_word_inputs)
        # char_word_inputs :  Tensor("inference/concat:0", shape=(?, ?, 400), dtype=float32)
        ...
        with tf.variable_scope("bi-RNN"):
            ...
            print("final_hiddens : ", final_hiddens)
            # final_hiddens :  Tensor("inference/bi-RNN/concat:0", shape=(?, 512), dtype=float32)
    ...
    def train(self):
        ...
        with sess.as_default():
            ...
            # tf.Print(self.build_graph.(logits).shape)  어떻게 하지.. ㅠㅠ
            ...
            print("inputs_id : ", inputs_id[0])
            print("labels_id : ", labels_id[0])
            print("chars_id  : ", chars_id[0])
            # inputs_id :  [21]
            # labels_id :  [0]
            # chars_id  :  [[23, 54, 48, 31, 27, 30, 43, 39, 30, 23]]
