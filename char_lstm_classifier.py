...
class TextClassifier:
    def __init__(self, hparams, data_dir):
        ...
        with np.load(os.path.join(self.hparams.glove_dir, "glove.6B.300d.trimmed.npz")) as pretrained_data:
            self.word_embedding_matrix = pretrained_data["embeddings"]
            print(np.shape(self.word_embedding_matrix))
    ...
    def _inference(self, inputs: tf.Tensor, lengths: tf.Tensor, char_inputs: tf.Tensor, char_lengths: tf.Tensor):
        ...
        with tf.variable_scope("char-bi-RNN"):
            ...
            print("char_hiddens_concatenated: ", char_hiddens)
            char_hiddens = tf.reshape(char_hiddens,
                                      [tf.shape(char_inputs)[0],
                                       tf.shape(char_inputs)[1],
                                       self.hparams.char_embedding_dim*2
                                       ])
            print("char_hiddens_concatenated_after_reshaped: ", char_hiddens)
        ...
        word_embedded = tf.nn.embedding_lookup(word_embedding_matrix, inputs)
    ...
    def _inference_original(self, inputs: tf.Tensor, lengths: tf.Tensor, char_inputs: tf.Tensor, char_lengths: tf.Tensor):
        print("Building graph for model: Text Classifier")
    ...
    def build_graph(self):
        ...
        with tf.name_scope("cross_entropy"):
            loss_op = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=self.labels_ph,
                                                                     name="cross_entropy")
            print("self.labels_ph: ", self.labels_ph)
        ...
        with sess.as_default():
            ...
            for epochs_completed in range(self.hparams.num_epochs):
                ...
                for iter in range(total_batch):
                    ...
                    print("batch_inputs: ", batch_data[0])
                    print("batch_labels: ", batch_data[1])
                    print("batch_lengths: ", batch_data[2])
                    print("batch_char_inputs: ", batch_data[3])
                    print("batch_char_lengths: ", batch_data[4])
